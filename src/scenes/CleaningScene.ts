...
export class CleaningScene extends Phaser.Scene {
    ...
    if (cosmeticId) {
      ...
      if (cosmeticDef) {
        ...
        if (this.textures.exists(cosmeticKey)) {
          ...
          const cosmetic = this.add.image(cx, cy, cosmeticKey).setScale(0.15);
  ...
  private createDirtPiles(): void {
    ...
    for (let i = 0; i < 12; i++) {
      dirtPositions.push({
        x: 80 + Math.random() * (GAME_WIDTH - 350),
        y: floorY + 20 + Math.random() * 100,
    ...
    dirtPositions.forEach((pos) => {
      ...
      (dirt as any)._radius = pos.size + 10;
  ...
  private createBroom(): void {
    this.broom = this.add.image(150, GAME_HEIGHT / 2 - 20, 'tool-broom')
      .setScale(0.20)
  ...
  private animalBounce(): void {
    ...
    const heart = this.add.image(
      this.animalContainer.x + Phaser.Math.Between(-20, 20),
      this.animalContainer.y - 60,
      'ui-heart',
    ).setScale(0.03);
    this.tweens.add({
      targets: heart,
  ...
  private showCompletion(result: StarAwardResult): void {
    for (let i = 0; i < 15; i++) {
      const sparkle = this.add.image(
        100 + Math.random() * (GAME_WIDTH - 300),
        GAME_HEIGHT - 150 + Math.random() * 120,
        'ui-sparkle',
    ...
    const txt = this.add.text(GAME_WIDTH / 2, GAME_HEIGHT / 2 - 60, 'Spotless!', {
      fontSize: '56px',
      fontFamily: 'Fredoka, Arial, sans-serif',
      fontStyle: 'bold',
      color: '#FFD700',
